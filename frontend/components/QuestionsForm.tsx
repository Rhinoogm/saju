...
export function QuestionsForm({
  ...
  eyebrow = "2단계",
  title = "마음의 방향 확인",
  description = "답변은 최종 풀이에서 사용자가 실제로 듣고 싶어 하는 결론을 잡는 데 쓰입니다.",
  submitLabel = "최종 풀이 보기",
  loadingLabel = "무료 모델 응답 대기 중",
  optionalQuestionIds = [],
  onBack,
  onAnswerChange,
  onSubmit,
  ...
  const optionalQuestionIdSet = new Set(optionalQuestionIds);
  const requiredAnswers = answers.filter((answer) => !optionalQuestionIdSet.has(answer.question_id));
  const isComplete = requiredAnswers.length === questions.length - optionalQuestionIds.length && requiredAnswers.every((answer) => answer.answer.trim().length > 0);
  ...
  return (
    <form onSubmit={handleSubmit} className="rounded-lg border border-stone-200 bg-white p-4 shadow-soft sm:p-6">
      <div className="mb-5 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <p className="text-sm font-black text-mint">{eyebrow}</p>
      ...
      <div className="space-y-4">
        {questions.map((question, index) => {
          const answer = answers.find((item) => item.question_id === question.id);
          const optional = optionalQuestionIdSet.has(question.id);
            ...
            <section key={question.id} className="rounded-lg border border-stone-200 bg-cloud p-4">
              <div className="mb-3 flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="mt-1 text-xs font-bold text-stone-500">
                    {question.type === "single_choice" ? "선택하거나 직접 입력" : optional ? "비워두어도 됩니다" : "짧게 입력"}
                  </p>
                </div>
              </div>
              ...
              {question.type === "single_choice" ? (
                ...
                <textarea
                  ...
                  placeholder={optional ? "더 남기고 싶은 말이 있다면 적어주세요." : "떠오르는 답을 짧게 적어주세요."}
                  required={!optional}
                />
              )}
            </section>
