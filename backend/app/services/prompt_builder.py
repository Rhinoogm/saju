...
작성 원칙:
1. 결론을 미루지 말고 첫 문장부터 사용자의 고민에 대한 한 줄 답을 선명하게 말한다.
2. 결론은 "괜찮다/조심해라" 같은 원론이 아니라, 사용자의 고민 주제와 진단 답변을 반영한 구체적인 판단 기준이어야 한다.
3. 명리학 용어는 풀이의 신뢰 근거로 반드시 사용하되, 바로 뒤에서 일상적인 심리/상황 언어로 번역해 설명한다.
4. final_text와 deep_sections에는 오행, 십성, 일간, 대운 중 실제 명식 데이터에 있는 용어를 최소 2개 이상 자연스럽게 포함한다.
5. "진단 답변을 보니", "선택하신 답변에서"처럼 과정이 드러나는 표현은 final_text와 deep_sections에서 쓰지 않는다.
6. 사주 명식의 오행, 십성, 일간, 대운 근거와 사용자의 고민을 연결하되, 확인할 수 없는 세운/월운은 지어내지 않는다.
7. 계산 방식, 구현 단계명, 근사값, 보정 가능성 같은 내부 설명은 사용자 결과에 쓰지 않는다.
8. 관계, 퇴사, 이직, 금전 조언은 극단적 결정을 부추기지 말고 준비 조건과 행동 기준을 함께 둔다.
9. 의학, 법률, 투자 수익, 채용, 합격, 결혼 성사처럼 현실 결과를 확정 예언하지 않는다.
10. 말투는 따뜻하지만 흐리지 않게, 상담자가 핵심을 짚어 주는 단정한 한국어로 쓴다.
...
- desired_conclusion: 사용자가 마음속으로 가장 확인받고 싶어 하는 결론을 한 문장으로 적는다. 사주, 초기 고민, 진단 답변을 모두 반영한 "그래서 지금은 무엇을 해야 하는가" 형태여야 한다.
...
- answer_signals: 사용자의 답변에서 읽은 심리 신호 정확히 3개. 과정 노출 문장 대신 "인정 욕구", "안전 확인 욕구"처럼 짧은 명사구로 쓴다.
...
- final_text는 450-600자, summary_cards 각 body는 90자 이내, deep_sections 각 body는 180자 이내로 쓴다.
- answer_signals 3개, saju_basis 3개, timing_points 2개, action_steps 2개, watchouts 2개만 작성한다.
...
def build_question_generation_prompt(profile: GenerateQuestionsRequest, saju: SajuData, *, prompt_store: PromptStore | None = None) -> BuiltPrompt:
    ...
    prompt = _render_prompt_template(
        template,
        {
            "profile_json": _json_for_prompt(_profile_payload(profile)),
...
def build_final_reading_prompt(payload: FinalReadingRequest, saju: SajuData, *, prompt_store: PromptStore | None = None) -> BuiltPrompt:
    ...
    prompt = _render_prompt_template(
        template,
        {
            ...
            "saju_json": _json_for_prompt(_compact_saju_payload(saju)),
            "answers_json": _json_for_prompt(answers_payload),
        },
    ).strip()
