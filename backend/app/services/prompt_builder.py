...
PREVIOUS_FINAL_USER_PROMPT_TEMPLATE = """아래 입력을 분석해 10~40대 여성이 SNS에 공유하고 싶어 하는 프리미엄 멘탈 케어 사주 결과지를 작성하라.
...
STORED_FINAL_USER_PROMPT_TEMPLATE_V1 = """아래 입력을 분석해 SNS에 공유하고 싶은 프리미엄 멘탈 케어 사주 결과지를 작성하라.
...
결과지 구성:
- reading_title: 결과지 맨 위 감성 타이틀. 16-28자 안팎으로, "완벽주의에 지친 당신을 위한 처방전"처럼 사용자의 현재 마음을 바로 비추는 제목으로 쓴다.
- core_message: 첫 화면에 크게 보이는 핵심 문장. 한 문장으로, "당신이 지금 힘든 건 약해서가 아니라 책임의 기운이 과열됐기 때문이에요"처럼 정당성(Validation)을 준다.
- hashtags: 정확히 4개. 모두 #으로 시작하고, 띄어쓰기 없이 10자 안팎의 한국어 감성 키워드로 쓴다. 예: "#책임감과다", "#번아웃직전".
- warm_hug: title은 "내 마음 돋보기"로 고정한다. headline은 현재 감정을 한 줄로 읽어주고, body는 사용자의 고민과 답변에서 드러난 마음을 3문장으로 다정하게 안아준다. 반드시 "당신 탓이 아니라" 또는 그에 준하는 표현으로 감정의 정당성을 부여하되, 불안 조장은 금지한다.
- saju_vibe: title은 "나만의 고유한 바이브"로 고정한다. headline은 일간과 핵심 오행을 활용한 물상 메타포 한 문장으로 쓴다. 예: "한겨울에 피어난 단단한 동백꽃 같은 사람". body는 해당 메타포가 성격, 관계, 선택 방식에 어떻게 나타나는지 쉬운 말로 설명한다.
- secret_talent: title은 "숨겨진 무기"로 고정한다. headline은 사용자가 단점으로 느끼는 특성을 재해석한 강점 한 줄로 쓴다. body는 예민함, 완벽주의, 망설임, 책임감 같은 부담을 실제 재능과 행동 전략으로 바꿔준다.
- timing_points: 정확히 3개. 앞으로 2주, 한 달, 1-3개월의 흐름을 각각 한 문장으로 쓴다. 확인되지 않은 특정 날짜·월·연도는 지어내지 말고, 실행 가능한 리듬으로 표현한다.
- luck_recipe: 화면의 "행운의 레시피" 영역에 들어갈 정확히 4개. category는 순서대로 "컬러", "음식", "작은 습관", "아이템"을 사용한다. item은 오늘 바로 고를 수 있는 구체적인 대상, reason은 그 대상이 사용자의 기운을 어떻게 부드럽게 돕는지 한 문장으로 쓴다.
- answer_signals: 사용자의 답변에서 읽은 심리 신호 정확히 3개. "인정 욕구", "안전 확인 욕구"처럼 짧은 명사구로 쓴다.
- saju_basis: 전문가 차트 토글 안에 들어갈 명식 근거 정확히 3개. 실제 명식 데이터의 일간, 오행, 십성, 대운 중 사용한 근거를 쉬운 해석과 함께 적는다.
- watchouts: 피해야 할 판단 방식이나 흔들릴 지점 정확히 2개. 사용자를 겁주지 말고 부드러운 경계선으로 쓴다.
- caution: 확정 예언이 아니라 자기 이해와 선택을 돕는 참고용 리딩이라는 안전 문장을 짧게 적는다.

품질 기준:
- 1040 여성 사용자가 "내 마음을 알아준다", "친구에게 캡처해서 보내고 싶다"고 느끼는 감성적이고 선명한 문장으로 쓴다.
- "당신은 이렇게 될 운명"이 아니라 "지금 이렇게 느끼는 이유가 있다"는 방식으로 결론을 재정의한다.
- 사주 용어는 전문가 차트 근거에는 사용할 수 있지만, 본문에서는 반드시 일상어로 번역한다.
- 바넘 효과처럼 누구에게나 맞는 말은 금지한다. 초기 고민, 답변, 명식 근거가 최소 2곳 이상에 구체적으로 연결되어야 한다.
- 무속적 표현, 저주/액운/살, 특정 사건의 확정 예언, 결제나 상담 유도 문구는 절대 쓰지 않는다.
- 계산 방식이나 구현 단계명 같은 내부 설명을 사용자 문장에 섞지 않는다.
"""
...
[1단계: 신뢰 형성 및 미러링]
- situation_mirror: title은 "당신의 현재 주파수". headline은 질문 답변에서 도출된 현재 상태 1줄 요약. body는 사용자가 선택한 [고정/맞춤 질문 답변]의 내용을 구체적으로 언급하며, 현재 사용자가 어떤 목표나 마음가짐을 갖고 있는지 거울처럼 비춰주는 분석을 3문장으로 작성한다.
...
- clear_solution: title은 "명쾌한 처방전". headline은 초기 고민(initial_concern)에 대한 직접적인 O/X 또는 행동 방향성 1줄. body는 구체적으로 지금 어떻게 행동해야 하는지(혹은 마음을 먹어야 하는지) 페르소나의 강력한 목소리로 3~4문장으로 찔러준다.

[3단계: 자아 탐색 및 팁 (공유용)]
- saju_vibe: title은 "나만의 고유한 바이브". headline은 일간/오행을 활용한 감성적 물상 메타포. body는 이 기운이 주는 본질적 매력 2문장.
- secret_talent: title은 "숨겨진 무기". headline은 단점으로 보일 수 있는 특성을 강점으로 리프레이밍. body는 이를 활용하는 전략 2문장.
- luck_recipe: "행운의 레시피". 정확히 4개. (category: "컬러", "음식", "작은 습관", "아이템"). 당장 실천 가능한 item과 그것이 사주에 어떻게 좋은지 설명하는 reason(1문장).

[4단계: 타이밍 및 재방문 훅]
- timing_points: 정확히 3개. 앞으로 2주, 한 달, 1-3개월 이내에 다가올 심리적/상황적 변화나 타이밍을 각각 한 문장으로 제시.
...
- 사주 용어는 'saju_basis'를 제외한 모든 본문에서 일상어로 완벽히 번역하라.
"""
...
- situation_mirror.body, saju_insight.body는 각각 120-280자로 쓴다.
- clear_solution.body는 140-320자로 쓴다.
- saju_vibe.body, secret_talent.body는 각각 80-180자로 쓴다.
- re_engagement_hook.body는 80-180자로 쓴다.
- hashtags 4개, answer_signals 3개, saju_basis 3개, timing_points 3개, luck_recipe 4개만 작성한다.
